...
static const int g_animation_timer =
    (int)((1.8f / g_fps) * 1000 - g_frame_time);
...
int main(int argc, char **argv) {
    try {
        int n = 0;
        std::cout << "Enter which demo to show:\n ";
        std::cout << "Press 1 for drop and 1 for hang demo\n";
        ...
        /*
        Inits 3 shaders :
        basic : Purpose (don't know)
        phong : For shading
        pick_frag : (don't know)
        */
        initCloth();
        initScene();
...
static void initCloth() {
    ...
    g_render_target->setNormalData(g_clothMesh->nbuff(),
                                   g_clothMesh->nbuffLen());
    g_render_target->setTextureData(g_clothMesh->tbuff(),
                                    g_clothMesh->tbuffLen());
    g_render_target->setIndexData(g_clothMesh->ibuff(),
                                  g_clothMesh->ibuffLen());
...
static void demo_hang() {
    ...
    massSpringBuilder.uniformGrid(
        SystemParam::n, SystemParam::h, SystemParam::r, SystemParam::k,
        SystemParam::m, SystemParam::a, SystemParam::g);
...
static void demo_drop() {
    ...
    CgSphereCollisionNode *sphereCollisionNode = new CgSphereCollisionNode(
        g_system, g_clothMesh->vbuff(), radius, center);
